...
export const assignUsersToTask = async (taskId: string, userIds: string[]) => {
  ...
  try {
    ...
    const usersCheck = await client.query('SELECT id FROM users WHERE id = ANY($1)', [userIds]);
    if (usersCheck.rows.length !== userIds.length) {
      throw new NotFoundError('One or more users not found');
    }

    await client.query('DELETE FROM task_assignments WHERE task_id = $1 AND user_id = ANY($2)', [
      taskId,
      userIds,
    ]);
